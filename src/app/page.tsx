'use client'
// src/app/page.tsx
// Frontend completo — consome as APIs REST do Next.js (Prisma + PostgreSQL)
...
export default function App() {
  ...
  // form refs
  const vf = useRef<Record<string, HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement | null>>({})
  const af = useRef<Record<string, HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement | null>>({})
  const mf = useRef<Record<string, HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement | null>>({})
  const df = useRef<Record<string, HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement | null>>({})
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      showToast('Erro ao carregar dados. Verifique a conexão com o banco.', true)
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  useEffect(() => { if (data && page === 'dashboard') renderCharts() }, [data, page])
  ...
  function renderCharts() {
    ...
    const destroy = (key: string) => { if (chartsRef.current[key]) { chartsRef.current[key].destroy(); delete chartsRef.current[key] } }
    ...
    destroy('rc')
    const rc = document.getElementById('chartRC') as HTMLCanvasElement
    ...
    // lucro
    const lucM = months.map((_, i) => recM[i] - desM[i])
    destroy('lc')
    const lc = document.getElementById('chartLC') as HTMLCanvasElement
    if (lc) chartsRef.current.lc = new w.Chart(lc, { type: 'line', data: { labels, datasets: [{ data: lucM, borderColor: '#3b82c4', backgroundColor: 'rgba(59,130,196,0.08)', fill: true, tension: 0.4, pointBackgroundColor: '#3b82c4', pointRadius: 4 }] }, options: opts(v => 'R$' + v.toLocaleString('pt-BR')) })

    // por veículo
    destroy('vei')
    const vc = document.getElementById('chartVei') as HTMLCanvasElement
    if (vc && veiculos.length) {
      const vData = veiculos.map(v => ({ l: v.placa, n: alugueis.filter(a => a.veiculoId === v.id && a.status === 'CONCLUIDO').length }))
      chartsRef.current.vei = new w.Chart(vc, { type: 'doughnut', data: { labels: vData.map(v => v.l), datasets: [{ data: vData.map(v => v.n), backgroundColor: COLORS }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, cutout: '60%' } })
    }

    // manutenção
    const prevM = months.map(mo => manutencoes.filter(m => m.tipo === 'PREVENTIVA' && getMonthKey(m.data) === mo.key).reduce((s, m) => s + m.custo, 0))
    const corrM = months.map(mo => manutencoes.filter(m => m.tipo === 'CORRETIVA' && getMonthKey(m.data) === mo.key).reduce((s, m) => s + m.custo, 0))
    destroy('mn')
    const mn = document.getElementById('chartMn') as HTMLCanvasElement
    if (mn) chartsRef.current.mn = new w.Chart(mn, { type: 'bar', data: { labels, datasets: [{ label: 'Preventiva', data: prevM, backgroundColor: '#f5a623', borderRadius: 4 }, { label: 'Corretiva', data: corrM, backgroundColor: '#e05252', borderRadius: 4 }] }, options: opts(v => 'R$' + v.toLocaleString('pt-BR')) })
  }
  ...
  function getVal(refs: React.MutableRefObject<Record<string, any>>, key: string) { return refs.current[key]?.value || '' }
  function setVal(refs: React.MutableRefObject<Record<string, any>>, key: string, val: string) { if (refs.current[key]) refs.current[key].value = val }
  ...
  async function saveVeiculo() {
    ...
    setSaving(true)
    try {
      editId ? await apiCall(`/api/veiculos/${editId}`, 'PUT', body) : await apiCall('/api/veiculos', 'POST', body)
      setModal(null); await loadData(); showToast('Veículo salvo!')
  ...
  async function saveAluguel() {
    ...
    setSaving(true)
    try {
      editId ? await apiCall(`/api/alugueis/${editId}`, 'PUT', body) : await apiCall('/api/alugueis', 'POST', body)
      setModal(null); await loadData(); showToast('Aluguel registrado!')
    } catch (e: any) { showToast(e.message, true) }
    setSaving(false)
  }
  ...
  async function saveManutencao() {
    ...
    setSaving(true)
    try {
      editId ? await apiCall(`/api/manutencoes/${editId}`, 'PUT', body) : await apiCall('/api/manutencoes', 'POST', body)
      setModal(null); await loadData(); showToast('Manutenção registrada!')
    } catch (e: any) { showToast(e.message, true) }
    setSaving(false)
  }
  ...
  async function saveDespesa() {
    ...
    setSaving(true)
    try {
      editId ? await apiCall(`/api/lancamentos/${editId}`, 'PUT', body) : await apiCall('/api/lancamentos', 'POST', body)
      setModal(null); await loadData(); showToast('Despesa registrada!')
    } catch (e: any) { showToast(e.message, true) }
    setSaving(false)
  }
  ...
  async function deleteItem(store: string, id: string) {
    ...
    try {
      ...
      await loadData(); showToast('Registro removido.')
    } catch (e: any) { showToast(e.message, true) }
  }
                      ...
                      <div className="table-wrapper">
                        <table>
                          ...
                          <tbody>{veiculos.length === 0 ? <tr><td colSpan={7}><div className="empty-state"><i className="ti ti-car-off"></i><p>Nenhum veículo cadastrado</p></div></td></tr> : veiculos.map(v => (
                            <tr key={v.id}><td className="fw-semibold nowrap">{v.placa}</td><td>{v.modelo}</td><td>{v.ano || '—'}</td><td>{v.cor || '—'}</td><td className="nowrap">{BRL(v.custo)}</td><td><Badge s={v.status} /></td><td><div className="td-actions"><button className="btn btn-secondary btn-sm btn-icon" onClick={() => openModal('veiculo', v.id)}><i className="ti ti-edit"></i></button><button className="btn btn-danger btn-sm btn-icon" onClick={() => deleteItem('veiculos', v.id)}><i className="ti ti-trash"></i></button></div></td></tr>
                          ))}</tbody>
                        </table>
                      </div>
                    </div>
                  ...
                  <div>
                    ...
                    <div className="card"><div className="table-wrapper"><table>
                      ...
                      <tbody>{alugueis.length === 0 ? <tr><td colSpan={8}><div className="empty-state"><i className="ti ti-route-off"></i><p>Nenhum aluguel registrado</p></div></td></tr> : alugueis.map(a => (
                        <tr key={a.id}><td className="nowrap">{fmtDate(a.data)}</td><td className="nowrap">{a.veiculo ? `${a.veiculo.placa} — ${a.veiculo.modelo}` : '—'}</td><td>{a.cliente}</td><td className="nowrap">{a.duracao ? `${a.duracao}h` : '—'}</td><td className="nowrap fw-semibold">{BRL(a.valor)}</td><td>{pagMap[a.pagamento] || a.pagamento}</td><td><Badge s={a.status} /></td><td><div className="td-actions"><button className="btn btn-secondary btn-sm btn-icon" onClick={() => openModal('aluguel', a.id)}><i className="ti ti-edit"></i></button><button className="btn btn-danger btn-sm btn-icon" onClick={() => deleteItem('alugueis', a.id)}><i className="ti ti-trash"></i></button></div></td></tr>
                      ))}</tbody>
                    </table></div></div>
                  </div>
                )}
                  ...
                  <div>
                    ...
                    <div className="card"><div className="table-wrapper"><table>
                      ...
                      <tbody>{manutFilt.length === 0 ? <tr><td colSpan={8}><div className="empty-state"><i className="ti ti-tool-off"></i><p>Nenhuma manutenção registrada</p></div></td></tr> : manutFilt.map(mn => (
                        <tr key={mn.id}><td className="nowrap">{fmtDate(mn.data)}</td><td className="nowrap">{mn.veiculo ? `${mn.veiculo.placa} — ${mn.veiculo.modelo}` : '—'}</td><td><Badge s={mn.tipo} /></td><td>{mn.descricao}</td><td>{mn.oficina || '—'}</td><td className="nowrap">{BRL(mn.custo)}</td><td><Badge s={mn.status} /></td><td><div className="td-actions"><button className="btn btn-secondary btn-sm btn-icon" onClick={() => openModal('manutencao', mn.id)}><i className="ti ti-edit"></i></button><button className="btn btn-danger btn-sm btn-icon" onClick={() => deleteItem('manutencoes', mn.id)}><i className="ti ti-trash"></i></button></div></td></tr>
                      ))}</tbody>
                    </table></div></div>
                  </div>
                )}
                  ...
                  <div>
                    ...
                    <div className="card"><div className="table-wrapper"><table>
                      ...
                      <tbody>{lancFilt.length === 0 ? <tr><td colSpan={6}><div className="empty-state"><i className="ti ti-coin-off"></i><p>Nenhuma movimentação</p></div></td></tr> : lancFilt.map(l => (
                        <tr key={l.id}><td className="nowrap">{fmtDate(l.data)}</td><td>{l.descricao}</td><td>{catMap[l.categoria] || l.categoria}</td><td><Badge s={l.tipo} /></td><td className={`nowrap fw-semibold ${l.tipo === 'RECEITA' ? 'text-green' : 'text-red'}`}>{l.tipo === 'RECEITA' ? '+' : '-'} {BRL(l.valor)}</td><td><div className="td-actions">{!l.aluguelId && !l.manutencaoId && <button className="btn btn-secondary btn-sm btn-icon" onClick={() => openModal('despesa', l.id)}><i className="ti ti-edit"></i></button>}<button className="btn btn-danger btn-sm btn-icon" onClick={() => deleteItem('lancamentos', l.id)}><i className="ti ti-trash"></i></button></div></td></tr>
                      ))}</tbody>
                    </table></div></div>
                  </div>
                )}
        ...
        <div className="modal-overlay open" onClick={e => e.target === e.currentTarget && setModal(null)}>
          <div className="modal">
            ...
            <div className="modal-body">
              <div className="form-row"><div className="form-group"><label>Placa *</label><input ref={el => vf.current.placa = el} placeholder="ABC-1234" /></div><div className="form-group"><label>Modelo *</label><input ref={el => vf.current.modelo = el} placeholder="Quadriciclo Sport 150" /></div></div>
              <div className="form-row"><div className="form-group"><label>Ano</label><input ref={el => vf.current.ano = el} type="number" placeholder="2024" /></div><div className="form-group"><label>Cor</label><input ref={el => vf.current.cor = el} placeholder="Vermelho" /></div></div>
              <div className="form-row"><div className="form-group"><label>Custo de aquisição (R$)</label><input ref={el => vf.current.custo = el} type="number" placeholder="8000" /></div><div className="form-group"><label>Status</label><select ref={el => vf.current.status = el}><option value="DISPONIVEL">Disponível</option><option value="ALUGADO">Alugado</option><option value="MANUTENCAO">Em manutenção</option><option value="INATIVO">Inativo</option></select></div></div>
              <div className="form-group"><label>Chassi</label><input ref={el => vf.current.chassi = el} /></div>
              <div className="form-group"><label>Observações</label><textarea ref={el => vf.current.obs = el as any}></textarea></div>
            </div>
      ...
      {modal === 'aluguel' && (
        <div className="modal-overlay open" onClick={e => e.target === e.currentTarget && setModal(null)}>
          <div className="modal">
            ...
            <div className="modal-body">
              <div className="form-row"><div className="form-group"><label>Data *</label><input ref={el => af.current.data = el} type="date" /></div><div className="form-group"><label>Veículo *</label><select ref={el => af.current.veiculo = el}><option value="">Selecione...</option>{veiculos.map(v => <option key={v.id} value={v.id}>{v.placa} — {v.modelo}</option>)}</select></div></div>
              <div className="form-row"><div className="form-group"><label>Cliente *</label><input ref={el => af.current.cliente = el} /></div><div className="form-group"><label>Contato</label><input ref={el => af.current.contato = el} placeholder="(85) 9 9999-9999" /></div></div>
              <div className="form-row"><div className="form-group"><label>Duração (h)</label><input ref={el => af.current.duracao = el} type="number" step="0.5" /></div><div className="form-group"><label>Valor (R$) *</label><input ref={el => af.current.valor = el} type="number" /></div></div>
              <div className="form-row"><div className="form-group"><label>Pagamento</label><select ref={el => af.current.pagamento = el}><option value="PIX">PIX</option><option value="DINHEIRO">Dinheiro</option><option value="CARTAO_DEBITO">Cartão Débito</option><option value="CARTAO_CREDITO">Cartão Crédito</option></select></div><div className="form-group"><label>Status</label><select ref={el => af.current.status = el}><option value="CONCLUIDO">Concluído</option><option value="ANDAMENTO">Em andamento</option><option value="CANCELADO">Cancelado</option></select></div></div>
              <div className="form-group"><label>Rota / Destino</label><input ref={el => af.current.rota = el} /></div>
            </div>
      ...
      {modal === 'manutencao' && (
        <div className="modal-overlay open" onClick={e => e.target === e.currentTarget && setModal(null)}>
          <div className="modal">
            ...
            <div className="modal-body">
              <div className="form-row"><div className="form-group"><label>Data *</label><input ref={el => mf.current.data = el} type="date" /></div><div className="form-group"><label>Veículo *</label><select ref={el => mf.current.veiculo = el}><option value="">Selecione...</option>{veiculos.map(v => <option key={v.id} value={v.id}>{v.placa} — {v.modelo}</option>)}</select></div></div>
              <div className="form-row"><div className="form-group"><label>Tipo *</label><select ref={el => mf.current.tipo = el}><option value="PREVENTIVA">Preventiva</option><option value="CORRETIVA">Corretiva</option></select></div><div className="form-group"><label>Status</label><select ref={el => mf.current.status = el}><option value="CONCLUIDA">Concluída</option><option value="AGENDADA">Agendada</option><option value="ANDAMENTO">Em andamento</option></select></div></div>
              <div className="form-group"><label>Descrição *</label><textarea ref={el => mf.current.desc = el as any}></textarea></div>
              <div className="form-row"><div className="form-group"><label>Oficina</label><input ref={el => mf.current.oficina = el} /></div><div className="form-group"><label>Custo (R$)</label><input ref={el => mf.current.custo = el} type="number" /></div></div>
              <div className="form-group"><label>Próxima revisão</label><input ref={el => mf.current.proxima = el} placeholder="3000 km ou 01/06/2025" /></div>
            </div>
      ...
      {modal === 'despesa' && (
        <div className="modal-overlay open" onClick={e => e.target === e.currentTarget && setModal(null)}>
          <div className="modal">
            ...
            <div className="modal-body">
              <div className="form-row"><div className="form-group"><label>Data *</label><input ref={el => df.current.data = el} type="date" /></div><div className="form-group"><label>Categoria</label><select ref={el => df.current.categoria = el}><option value="COMBUSTIVEL">Combustível</option><option value="SEGURO">Seguro</option><option value="LICENCIAMENTO">Licenciamento</option><option value="PONTO">Ponto/Aluguel</option><option value="MARKETING">Marketing</option><option value="PESSOAL">Pessoal</option><option value="EQUIPAMENTO">Equipamento</option><option value="OUTROS">Outros</option></select></div></div>
              <div className="form-group"><label>Descrição *</label><input ref={el => df.current.desc = el} /></div>
              <div className="form-row"><div className="form-group"><label>Valor (R$) *</label><input ref={el => df.current.valor = el} type="number" /></div><div className="form-group"><label>Veículo (se aplicável)</label><select ref={el => df.current.veiculo = el}><option value="">Geral</option>{veiculos.map(v => <option key={v.id} value={v.id}>{v.placa}</option>)}</select></div></div>
            </div>
